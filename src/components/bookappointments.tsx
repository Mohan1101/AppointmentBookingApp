...
export default function BookAppointments({
    ...
    const handleCheckout = async () => {
        try {
            if (process.env.NEXT_PUBLIC_API_KEY === undefined || process.env.NEXT_PUBLIC_API_KEY === null) {
            ...
            const data = {
                lineItems: [
                    {
                        priceId: 'price_1P7CMhSCxpUA8VZSK9oH1IOx',
                        quantity: 1,
                    },
                ],
                customerInfo: {
            ...
            // Pass the data to the backend as body
            const response = await axios.post('/api/checkout', data);

            const sessionData = response.data;
            console.log('Session data:', sessionData);

            if (response.status === 200 && sessionData.sessionId) {
                if (!stripe) {
    ...
    const handleBookAppointment = async () => {
        if (!isAuthenticated) {
            ...
            try {
                // Call the checkout function first to initiate payment
        ...
        <div>
            ...
            <section className="bg-dark-900">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
                    ...
                    {/* Hero content */}
                    <div className="relative flex flex-col items-center gap-12 md:flex-row-reverse pt-32 pb-10 md:pt-40 md:pb-16">
                        {/* Section header */}
                        <div
                            data-aos="fade-up"
                            data-aos-delay="200"
                            className="w-full md:w-1/2 border rounded-lg bg-white max-w-3xl mx-4 text-center text-gray-500 pb-12 md:pb-16"
                        >
                            <div className="flex flex-col gap-12 items-center text-sm p-12 font-semibold">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    {slotId && (
                                        <p className="mt-4 text-green-500">You have chosen the {availableSlots.find(slot => slot._id === slotId)?.time} slot on {dayjs(selectedDate).format("DD/MM/YYYY")}</p>
                                    )
                                    }

                                    {/* Book Appointment button */}
                                    <Button className="bg-purple-600 hover:bg-purple-700 text-white" onClick={() => handleBookAppointment()}>
                                        Book Appointment
                                    </Button>
                                </div>
                            </div>
